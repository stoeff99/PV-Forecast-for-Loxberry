...
#done < $indexfile ; echo $line >> $logfilepath
    ...
    index=$(echo $indexline | cut -d" " -f1)
    value_UDP=$($scriptsdir/./can_scan can0 680 180.$index | grep "value" | replace ")" "" | replace "(" "" | replace "value:" "")
    value_UDP=$(echo $value_UDP | cut -d" " -f2,3)
    #echo "$value_UDP"
...
echo -n "value=$line_UDP" >/dev/udp/89.26.24.2/7006
